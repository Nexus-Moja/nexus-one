...
import {Ambulance, Accessibility, ArrowRight, Building2, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, ClipboardCopy, ExternalLink, FileCheck2, Info, Mail, MapPin, Menu, Pause, Phone, Play, Route, ShieldCheck, Users, Van, X} from 'lucide-react';
import './styles.css';
...
function App(){
 ...
 return <div>
  ...
  <header className="topbar"><div className="shell nav">
   ...
   <nav id="primary-navigation" aria-label="Primary navigation" className={mobile?'navlinks open':'navlinks'}>{['Services','Patient Experience','Coverage','Facilities','Safety & Quality'].map(x=><a key={x} href={'#'+x.toLowerCase().replaceAll(' ','-').replace('&','and')} onClick={()=>setMobile(false)}>{x}</a>)}</nav>
   <div className="navRight"><a className="trackBtn" href="/livecare.html">NEXUS LiveCare</a><a className="call" href="tel:+18887604990"><Phone size={18} aria-hidden="true"/><span><small>Call Nexus</small><b>(888) 760-4990</b></span></a><BookRideButton onBook={openBooking}>Book a Ride</BookRideButton><span className="nexusLanguageSlot" data-nexus-language-slot aria-label="Language selector"></span><button type="button" className="menuBtn" aria-label={mobile?'Close navigation menu':'Open navigation menu'} aria-expanded={mobile} aria-controls="primary-navigation" onClick={()=>setMobile(!mobile)}>{mobile?<X aria-hidden="true"/>:<Menu aria-hidden="true"/>}</button></div>
  </div></header>
  ...
  <footer><div className="shell footerGrid"><div className="footerBrand"><a className="brand footerLogoLink" href="#home" aria-label="Nexus Medical Transit home"><img className="nexusLogo footerNexusLogo" src="./nexus-footer-logo.png" alt="Nexus Medical Transit" /></a><p>Compassionate, timely and inclusive healthcare mobility across the Washington Metropolitan Area.</p></div><div><h4>Patients & Families</h4><button onClick={()=>openBooking()}>Book Transportation</button><button onClick={()=>setTrack(true)}>Track a Request</button><a href="#services">Services</a><a href="#safety-and-quality">Safety & Quality</a></div><div><h4>Healthcare Partners</h4><button onClick={()=>setPartner(true)}>Facility Partnership</button><a href="#coverage">Coverage</a><a href="#facilities">Facility Portal</a></div><div><h4>Contact</h4><a href="tel:+18887604990">(888) 760-4990</a><a href="mailto:contact@nexusmt.com">contact@nexusmt.com</a><a href="https://nexusmt.com" target="_blank" rel="noopener noreferrer">nexusmt.com <span className="srOnly">(opens in a new tab)</span></a><a href="./accessibility.html">Accessibility statement</a></div></div><div className="shell footerBottom"><span>© 2026 Nexus Medical Transit LLC</span><span>For emergencies, call 911.</span></div></footer>
...
const STRIPE_CHECKOUT_ENDPOINT='/api/payments/stripe/checkout';
...
function PaymentOptions({reference,fare,checkoutToken}){
 const paypalRef=useRef(null); const [paypalError,setPaypalError]=useState('');
 const [stripeError,setStripeError]=useState(''); const [stripeLoading,setStripeLoading]=useState(false);
 useEffect(()=>{
  let active=true;
 ...
 const startStripeCheckout=async()=>{
  ...
  try{
   const response=await fetch(STRIPE_CHECKOUT_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({reference,checkoutToken})});
   const result=await response.json();
   if(!response.ok||!result.url)throw new Error(result.error||'Unable to start Stripe Checkout.');
   window.location.assign(result.url);
  }catch(err){setStripeError(err.message)}finally{setStripeLoading(false)}
 };
 return <section className="paymentPanel" aria-labelledby="payment-heading">
  ...
  <p>Your secure checkout is connected to Nexus confirmation <b>{reference}</b>. The final trip charge remains subject to dispatch confirmation.</p>
  <div className="paymentGrid">
   <button type="button" className="paymentMethod stripePay" onClick={startStripeCheckout} disabled={stripeLoading||!checkoutToken}><span><b>{stripeLoading?'Opening secure checkout…':'Card, Apple Pay or Google Pay'}</b><small>Secure Checkout Session powered by Stripe</small></span><ExternalLink size={18} aria-hidden="true"/></button>
  ...
  {stripeError&&<div className="apiError" role="alert">{stripeError}</div>}
